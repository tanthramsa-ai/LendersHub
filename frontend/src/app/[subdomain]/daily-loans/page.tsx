...
import { getDailyLoans, getBranches, DailyLoan, TenantBranch, getTenantSession, LOAN_ROLES } from '@/services/tenant-api';
...
const CYCLE_LABELS: Record<string, string> = {
  DAILY_NO_SUNDAY: 'No Sun',
  DAILY_WITH_SUNDAY: 'All Days',
};

export default function DailyLoansPage() {
  ...
  const load = useCallback(async (p: number, s: string, st: string, br: string, ct: string) => {
    setLoading(true);
    try {
      const res = await getDailyLoans(p, limit, {
        search: s || undefined, status: st || undefined,
        branchId: br || undefined, cycleType: ct || undefined,
      });
      setLoans(res.data);
      setTotal(res.total);
  ...
  useEffect(() => { load(page, search, status, branchId, cycleType); }, [page, search, status, branchId, cycleType, load]);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Daily Loans</h1>
          <p className="text-sm text-gray-500">{total} loan{total !== 1 ? 's' : ''}</p>
        </div>
        {canCreate && (
          <Link href={`/${subdomain}/daily-loans/new`}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={cycleType} onChange={(e) => { setCycleType(e.target.value); setPage(1); }}
          className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
          <option value="">All Cycles</option>
          <option value="DAILY_NO_SUNDAY">Daily (No Sunday)</option>
          <option value="DAILY_WITH_SUNDAY">Daily (All Days)</option>
        </select>
                  ...
                  {loans.map((l) => (
                    <tr key={l.id} className={`hover:bg-gray-50 transition-colors ${l.isNpa ? 'bg-red-50' : ''}`}>
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/daily-loans/${l.id}`} className="text-blue-600 hover:underline font-mono">{l.loanNumber}</Link>
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3">
                        <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${l.cycleType === 'DAILY_NO_SUNDAY' ? 'bg-purple-100 text-purple-700' : 'bg-indigo-100 text-indigo-700'}`}>
                          {CYCLE_LABELS[l.cycleType] ?? l.cycleType}
                        </span>
                      </td>
                      <td className="px-3 py-3 font-medium text-gray-900 whitespace-nowrap">{fmt(l.principal)}</td>
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{l.emiAmount ? fmt(l.emiAmount) : '—'}</td>
                      ...
                      <td className="px-3 py-3 text-green-700 whitespace-nowrap">{fmt(l.principalReceived)}</td>
                      <td className="px-3 py-3 text-green-600 whitespace-nowrap">{fmt(l.interestReceived)}</td>
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/daily-loans/${l.id}`} className="text-xs text-blue-600 hover:underline whitespace-nowrap">View →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
