...
export default function NewMonthlyLoanPage() {
  ...
  async function handleAddCustomer() {
    if (!newCust.firstName || !newCust.phone || !newCust.address || !newCust.locality || !newCust.altContact) {
      setAddCustError('First name, phone, address, locality and alternate contact are required'); return;
    }
    if (!newCust.panNumber && !newCust.aadhaarLast4) {
      setAddCustError('At least PAN or Aadhaar number is required'); return;
    }
    ...
    try {
      ...
      setSelectedCustomer(result); setShowAddCustomer(false);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push(`/${subdomain}/monthly-loans/${result.id}`);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div>
        <Link href={`/${subdomain}/monthly-loans`} className="text-sm text-blue-600 hover:underline">← Back to Monthly Loans</Link>
        <h1 className="text-xl font-bold text-gray-900 mt-1">New Monthly Loan</h1>
        <p className="text-sm text-gray-500 mt-0.5">Interest-only monthly collection · Principal collected on loan closure</p>
      </div>
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800">
            <strong>Interest-only loan:</strong> Only the monthly interest is collected each month. The principal (₹{form.principal || '…'}) will be collected when the loan is closed.
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <input value={form.purpose} onChange={(e) => setF('purpose', e.target.value)} className={inputCls} placeholder="Business, Agriculture…" />
              </div>
            </div>
