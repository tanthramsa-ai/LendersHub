...
// Routes under this dynamic [subdomain] segment that are NOT tenant pages
const EXCLUDED = ['super-admin', 'api', 'tenant', 'login', 'register'];
...
export default function TenantRootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Don't render the tenant chrome on the login page
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="px-4 py-4 border-t border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { clearTenantSession(); router.push(`/${subdomain}/login`); }}
              title="Sign out"
