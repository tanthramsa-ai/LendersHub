...
const LOANS_GROUP_ITEMS: NavItem[] = [
  ...
  {
    href: 'daily-loans', label: 'Daily Loans', roles: CAN_CREATE_LOANS,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'monthly-loans', label: 'Monthly Loans', roles: CAN_CREATE_LOANS,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'agent-risk-loans', label: 'Agent Risk Loans', roles: CAN_CREATE_LOANS,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const NAV_BOTTOM: NavItem[] = [
  ...
  {
    href: 'collection-ledger', label: 'Collection Ledger', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'daily-ledger', label: 'Daily Ledger', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
  ...
  {
    href: 'funders', label: 'Funders', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'reconciliation', label: 'Reconciliation', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'payments', label: 'Direct Payments', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'accounts', label: 'Accounts', roles: MANAGER_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'collections', label: 'Collections', roles: CAN_COLLECT,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'users', label: 'Team', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'activity', label: 'Activity Log', roles: MANAGER_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {
    href: 'settings', label: 'Settings', roles: USER_ADMIN_ROLES,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function TenantLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Route guard: redirect to dashboard if role can't access current page.
    // Check sub-route overrides first (more specific), then fall back to NAV-level rules.
    const role = session.user.role as UserRole;
    const subroute = SUBROUTE_PERMISSIONS.find((r) => pathname.includes(r.suffix));
    if (subroute && !subroute.roles.includes(role)) {
