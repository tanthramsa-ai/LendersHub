...
import { getTermLoans, getBranches, TermLoan, TenantBranch, getTenantSession, LOAN_ROLES } from '@/services/tenant-api';
...
export default function TermLoansPage() {
  ...
  const load = useCallback(async (p: number, s: string, st: string, br: string) => {
    setLoading(true);
    try {
      const res = await getTermLoans(p, limit, { search: s || undefined, status: st || undefined, branchId: br || undefined });
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-gray-50">
                  <tr>
                    {['Loan ID','Customer','Branch','Principal','EMI','Rate','Term','Outstanding','Paid','Overdue','NPA','Status',''].map((h) => (
                ...
                <tbody className="divide-y divide-gray-100">
                  {loans.map((l) => (
                    <tr key={l.id} className={`hover:bg-gray-50 transition-colors ${l.isNpa ? 'bg-red-50' : l.status === 'CLOSED' ? 'bg-slate-50' : ''}`}>
                      ...
                      <td className="px-3 py-3">
                        <div className="font-medium text-gray-900 whitespace-nowrap">{l.customerName}</div>
                      ...
                      <td className="px-3 py-3 text-blue-700 font-medium whitespace-nowrap">{l.emi ? fmt(l.emi) : '—'}</td>
                      <td className="px-3 py-3 text-gray-500 whitespace-nowrap">{l.interestRate}%</td>
                      <td className="px-3 py-3 text-gray-500 whitespace-nowrap">{l.termMonths}mo</td>
                      <td className="px-3 py-3 font-medium text-red-600 whitespace-nowrap">{fmt(l.outstanding)}</td>
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/loans/${l.id}`} className="text-xs text-blue-600 hover:underline whitespace-nowrap">View →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
