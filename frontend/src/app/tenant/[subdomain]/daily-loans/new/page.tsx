...
import {
  ...
  Customer, TenantBranch, LoanType, DailySchedulePreview, WeeklyCalculationType, Officer,
...
export default function NewDailyLoanPage() {
  ...
  const [form, setForm] = useState({
    ...
    calculationType: 'FLAT' as WeeklyCalculationType,
    interestPerDay: '3.19',
    emiRounding: '0',
  });
  ...
  async function handlePreview() {
    ...
    if (!form.principal || !rateFilled || !form.termDays || !form.firstDueDate) {
      setPreviewError(isPerDay
        ? 'Fill principal, interest per ₹1,000/day, tenure and first due date'
        : 'Fill principal, interest rate, tenure and first due date');
      return;
    }
    setPreviewError('');
    setPreviewing(true);
              ...
              <div>
                {isPerDay ? (
                  <>
                    ...
                    {/* 365-day basis here — the daily engine divides the annual rate by 365,
                        unlike weekly's 7 × 52 = 364. */}
                    <p className="text-xs text-gray-400 mt-1">
                      ≈ {(parseFloat(form.interestPerDay || '0') * 36.5).toFixed(2)}% p.a. flat
                    </p>
                  </>
                ) : (
                  <>
                  ...
                  <div className="mt-2 max-h-64 overflow-y-auto border border-gray-200 rounded-lg">
                    <table className="w-full text-xs">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>
                          {['#', 'Due Date', 'Day', 'Principal', 'Interest', 'EMI', 'Outstanding'].map((h) => (
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
              {[
                ...
                { label: 'Cycle', value: form.cycleType === 'DAILY_NO_SUNDAY' ? 'Daily (No Sunday)' : 'Daily (All Days)' },
                {
                  label: 'Interest Rate',
                  value: isPerDay
                    ? `₹${form.interestPerDay} per ₹1,000/day (${(parseFloat(form.interestPerDay || '0') * 36.5).toFixed(2)}% p.a. flat)`
