...
import { getAgentRiskLoans, getBranches, AgentRiskLoan, TenantBranch, getTenantSession, LOAN_ROLES } from '@/services/tenant-api';
...
export default function AgentRiskLoansPage() {
  const params = useParams<{ subdomain: string }>();
  const subdomain = params.subdomain;
  const session = getTenantSession();
  const canCreate = LOAN_ROLES.includes(session?.user.role ?? 'VIEWER');

  const [loans, setLoans] = useState<AgentRiskLoan[]>([]);
  ...
  const load = useCallback(async (p: number, s: string, st: string, br: string) => {
    setLoading(true);
    try {
      const res = await getAgentRiskLoans(p, limit, { search: s || undefined, status: st || undefined, branchId: br || undefined });
      setLoans(res.data); setTotal(res.total);
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Agent Risk Loans</h1>
          <p className="text-sm text-gray-500">{total} loan{total !== 1 ? 's' : ''} · Interest-only · Agent guaranteed</p>
        </div>
        {canCreate && (
          <Link href={`/${subdomain}/agent-risk-loans/new`}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-gray-400 text-sm mb-3">No agent risk loans found</p>
            {canCreate && <Link href={`/${subdomain}/agent-risk-loans/new`} className="text-sm text-blue-600 hover:underline">Create first agent risk loan</Link>}
          </div>
        ) : (
          <>
                  ...
                  {loans.map((l) => (
                    <tr key={l.id} className={`hover:bg-gray-50 transition-colors ${l.isNpa ? 'bg-red-50' : ''}`}>
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/agent-risk-loans/${l.id}`} className="text-blue-600 hover:underline font-mono">{l.loanNumber}</Link>
                      </td>
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/customers/${l.customerId}`} className="font-medium text-gray-900 hover:text-blue-600 whitespace-nowrap">{l.customerName}</Link>
                      ...
                      <td className="px-3 py-3">
                        {l.overdueCount > 0 && <span className="px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-xs font-medium">{l.overdueCount} OD</span>}
                      </td>
                      <td className="px-3 py-3">
                        {l.isNpa
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/agent-risk-loans/${l.id}`} className="text-xs text-blue-600 hover:underline whitespace-nowrap">View →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
