...
import { getWeeklyLoans, getBranches, WeeklyLoan, TenantBranch, getTenantSession, LOAN_ROLES } from '@/services/tenant-api';
...
export default function WeeklyLoansPage() {
  ...
  const load = useCallback(async (p: number, s: string, st: string, br: string) => {
    setLoading(true);
    try {
      const res = await getWeeklyLoans(p, limit, { search: s || undefined, status: st || undefined, branchId: br || undefined });
      setLoans(res.data);
      setTotal(res.total);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Weekly Loans</h1>
          <p className="text-sm text-gray-500">{total} loan{total !== 1 ? 's' : ''}</p>
        </div>
        {canCreate && (
          <Link href={`/${subdomain}/weekly-loans/new`}
                ...
                <tbody className="divide-y divide-gray-100">
                  {loans.map((l) => (
                    <tr key={l.id} className={`hover:bg-gray-50 transition-colors ${l.isNpa ? 'bg-red-50' : l.status === 'CLOSED' ? 'bg-slate-50' : ''}`}>
                      ...
                      <td className="px-3 py-3 text-gray-500">{l.paidInstallments}/{l.totalInstallments}</td>
                      <td className="px-3 py-3">
                        {l.overdueCount > 0 && (
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/${subdomain}/weekly-loans/${l.id}`} className="text-xs text-blue-600 hover:underline whitespace-nowrap">View →</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
