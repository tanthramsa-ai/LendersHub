...
function LoginForm() {
  ...
  const [subdomain, setSubdomain] = useState(searchParams.get('org') ?? '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await tenantLogin(email, password, subdomain.toLowerCase().trim());
      saveTenantSession(res);
      router.push(`/tenant/${res.tenant.subdomain}/dashboard`);
    } catch (err) {
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 bg-gray-50">
      <div className="w-full max-w-md">
        ...
        <p className="text-gray-500 mb-8">Sign in to your organisation portal</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            />
            <p className="text-xs text-gray-400 mt-1">Your organisation&apos;s unique identifier</p>
          </div>
...
export default function TenantLoginPage() {
  return (
    <div className="min-h-screen flex">
      ...
      {/* Right panel — wrapped in Suspense because LoginForm uses useSearchParams */}
